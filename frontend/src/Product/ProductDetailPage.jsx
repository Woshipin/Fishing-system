...
const ProductDetailPage = () => {
  ...
  const addToCart = async () => {
    try {
      ...
      // 构建请求数据
      const payload = {
        user_id: parseInt(userId, 10), // 确保是整数
        product_id: product.id,
        name: product.name,
        quantity: quantity,
        price: product.price,
        slug: product.slug || "", // 如果没有slug，传空字符串
        category_id: product.category_id || null, // 如果没有category_id，传null
        image:
      ...
      const response = await axios.post(
        "http://127.0.0.1:8000/api/cart/product",
        payload,
        {
          headers: {
      ...
      if (response.data.success) {
        ...
        console.log("Cart item added:", response.data.cartItem);

        // 可选：更新购物车数量显示
        if (response.data.cart_count) {
          console.log("Total cart items:", response.data.cart_count);
          // 这里可以更新购物车图标的数量显示
        }
      } else {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (productData.image_urls && Array.isArray(productData.image_urls)) {
          ...
          imageUrls = productData.productImages.map(
        ...
        const formattedProduct = {
          ...
          reviewCount:
            ...
            (productData.reviews && Array.isArray(productData.reviews)
          ...
          relatedProducts: Array.isArray(productData.related_products)
            ? productData.related_products.map((rp) => {
                ...
                if (rp.image_urls && Array.isArray(rp.image_urls)) {
                  ...
                  rp.productImages &&
                  Array.isArray(rp.productImages)
                ) {
                  relatedImageUrls = rp.productImages.map(
                    (img) => `${BASE_IMAGE_URL}${img.image_path}`
                  );
