...
const FishingTimePage = () => {
  ...
  useEffect(() => {
    const fetchUserDurations = async () => {
      try {
        const response = await fetch("http://localhost:8000/api/user-durations");
        if (!response.ok) {
          throw new Error("Failed to fetch user durations");
        }
        const data = await response.json();

        // Transform the fetched data to match the existing structure
        const usersWithDurations = data.map((userDuration) => ({
          id: userDuration.user.id,
          name: userDuration.user.name,
          avatar: `https://via.placeholder.com/150?text=${userDuration.user.name}`,
          location: "Unknown Location", // You can modify this as needed
          startTime: new Date(userDuration.created_at),
          endTime: new Date(
            new Date(userDuration.created_at).getTime() +
              userDuration.duration.seconds * 1000
          ),
          fishCaught: Math.floor(Math.random() * 15), // Random fish caught for demonstration
        }));

        setFishingUsers(usersWithDurations);
      } catch (error) {
        ...
        prevUsers.map((user) => {
          ...
          if (user.endTime <= new Date()) {
            const newEndTime = new Date(
              Date.now() + (Math.random() * 90 + 30) * 60 * 1000
            );
            return {
              ...user,
              startTime: new Date(),
  ...
  const filteredUsers = () => {
    switch (filterOption) {
      case "critical":
        return fishingUsers.filter(
          (user) => getRemainingMinutes(user.endTime) <= 10
        );
      case "normal":
        return fishingUsers.filter(
          (user) => getRemainingMinutes(user.endTime) > 10
        ...
        <div className="container mx-auto px-4">
          <div className="mb-6 flex justify-between items-center">
            <h2 className="text-2xl font-bold">
              Active Fishers: {sortedUsers.length}
            </h2>
