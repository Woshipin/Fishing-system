...
const ImageWithFallback = ({ src, alt, className, ...props }) => {
  ...
  const imageSrcToRender =
    !currentSrc || hasError ? DEFAULT_IMAGE_URL : currentSrc;
  ...
  return (
    <div className={`flex ${className}`}>
      {[...Array(5)].map((_, index) => {
        ...
        return (
          <span key={index} className={`text-lg ${starClass}`}>
            ★
          </span>
        );
      })}
...
const PackageDetailPage = () => {
  ...
  const fetchPackageDetail = async () => {
    try {
      ...
      processedPackage.reviews = Array.isArray(data.package_reviews)
        ? data.package_reviews
            .map((review) => ({
              id: review.id,
      ...
      processedPackage.rating = data.package_reviews_avg_rating
        ? parseFloat(data.package_reviews_avg_rating).toFixed(1)
        : 0;
      processedPackage.reviewCount = data.package_reviews_count || 0;

      // 确保 price 和 originalPrice 是数字
      processedPackage.price = parseFloat(data.price) || 0;
      processedPackage.originalPrice = parseFloat(data.original_price) || 0;
  ...
  // ✅✅✅ FIX: 实现了完整的 handleAddToCart 函数 ✅✅✅
  const handleAddToCart = async () => {
    if (addingToCart || !packageData?.inStock) {
      return;
    }
    if (!userId) {
      showToast("You must log in to add items to your cart.", "error");
      return;
    }
    ...
    const payload = {
      ...
      name: packageData.title, // 前端叫 title, 后端验证叫 name
      category_id: packageData.category_id || null, // 确保 packageData 中有 category_id
      quantity: 1, // 套餐通常一次添加一个
      price: packageData.price,
      image: packageData.imageUrls?.[0] || null,
      features: packageData.features || null, // 假设 packageData 中有 features
    };
    ...
    try {
      ...
      if (response.ok) {
        showToast(data.message || "Package added to cart!", "success");
        // 可选: 更新全局购物车状态
      } else {
        // 处理后端返回的错误信息，包括验证错误
        if (data.errors) {
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/packages/${id}/reviews`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const totalReviewPages = Math.ceil(
    (packageData?.reviews.length || 0) / reviewsPerPage
  );

  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen text-xl font-semibold text-blue-700">
        Loading Package...
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 md:py-16 relative z-10">
        ...
        >
          ...
          <div className="space-y-4">
            ...
            >
              <ImageWithFallback
                key={imageUrls[selectedImageIndex] || id}
              ...
              <div className="mt-auto pt-6 border-t border-gray-200/80 space-y-6">
                ...
                <motion.div className="text-center">
                  <motion.button
                    ...
                    className={`w-full px-8 py-4 rounded-2xl font-bold text-lg transition-all duration-300 border-2 ${
                      packageData.inStock && !addingToCart
                        ? "text-white transform hover:scale-105 hover:cursor-pointer"
        ...
        {/* 包含的产品 */}
        <motion.div
          className="rounded-3xl p-6 md:p-10 mb-16"
          style={glowStyle}
          initial={{ opacity: 0, y: 50 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          {products.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
              {products.map((product, index) => (
                <motion.div
                  key={product.id}
                  className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 border transition-all duration-300"
                  style={{
                    boxShadow:
                ...
                >
                  ...
                  <h3 className="font-bold text-gray-800 mb-2 text-md line-clamp-2">
                    {product.name}
                  </h3>
            ...
            <div className="text-center py-12 text-gray-500">
              This package currently includes no products.
            </div>
          )}
        </motion.div>
          ...
          <div className="space-y-10">
              ...
              {userId ? (
                <form onSubmit={handleSubmitReview} className="space-y-4">
                  ...
                  <button
                    ...
                    style={buttonStyle}
                    className="px-6 py-2.5 rounded-lg text-white font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
                  >
            ...
            <div className="space-y-6">
              <h3 className="text-2xl font-bold text-gray-800">
                Customer Reviews ({packageData.reviewCount})
              </h3>
              {currentReviews.length > 0 ? (
                currentReviews.map((review) => (
                  <div
                    key={review.id}
                    className="p-5 border rounded-2xl bg-white/50"
                  >
                ...
                <p className="text-center text-gray-500 py-8">
                  No reviews for this package yet. Be the first!
                </p>
              )}
            </div>
            ...
            {totalReviewPages > 1 && (
              <div className="flex justify-center items-center space-x-4 pt-4">
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-white border border-gray-300 disabled:opacity-50 transition hover:bg-gray-50"
                >
                  Previous
                </button>
